...
def main():
    ...
    text = """
            ...
            L = Draw line
            S = Draw Square
            E = Draw Equilateral triange
            H = Rhombus
            C = Eraser
            A = Clear
            """
     # Color selection buttons (rectangles)
    r = pygame.Rect(30, 200, 30, 30)
    g = pygame.Rect(30, 250, 30, 30)
    b = pygame.Rect(30, 300, 30, 30)
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                #eraser
                    ...
                    drawing_mode = 4  # square  #  NEW
                elif event.key == pygame.K_r:
                    drawing_mode = 5  # right triangle  #  NEW
                elif event.key == pygame.K_e:
                    drawing_mode = 6  # equilateral triangle  #  NEW
                elif event.key == pygame.K_h:
                    drawing_mode = 7  # rhombus  #  NEW
                    #clear all
...
def drawfig(screen, index, start, end, width, color_mode, draw_mode):
    ...
    elif draw_mode == 4:  # NEW DRAW SQUARE
        size = max(abs(x2 - x1), abs(y2 - y1))
        rect = pygame.Rect(x1, y1, size, size)
        pygame.draw.rect(screen, color, rect, width)

    elif draw_mode == 5:  # NEW RIGHT TRIANGLE
        points = [(x1, y2), (x1, y1), (x2, y2)]
        pygame.draw.polygon(screen, color, points, width)
        
    elif draw_mode == 6:  # NEW EQUILATERAL TRIANGLE
        mid_x = (x1 + x2) // 2
        top = (mid_x, y1)
        left = (x1, y2)
        right = (x2, y2)
